...
Element.prototype.removeValidator = function(validator) {
    var validator = this.getValidator(validator);

    this._options.validators = _.reject(this._options.validators, validator);
};
...
Element.prototype.render = function(callback, options) {
    ...
    if (_.isFunction(callback))
    {
        ...
        if (this._render.htmlTag)
        {
            ...
            if (!_.isUndefined(this._options.value) && !_.isNull(this._options.value))
            {
                ...
                if (this.getIsArray() && _.isArray(value))
                    value = "___" + this.getFullyQualifiedName() + "_value___";
        ...
        if (this._render.hasCustomRenderChildren)
            html += "\r%%custom_render_children%%";

        if (this.getError())
        {
            var errorString = "\r.form-error " + this.getError();

            html += options.noRender ? errorString : jade.render(errorString);
        }
    ...
    {
        if (this.getIsArray() && _.isArray(this.getValue()))
        {
            if (_.isUndefined(this._valueIndex))
                this._valueIndex = 0;
            else
                this._valueIndex++;

            var pattern = entities.encodeHTML("___" + this.getFullyQualifiedName() + "_value___");
            var val = this.getValue()[this._valueIndex];

            if (_.isUndefined(val) || _.isNull(val))
                val = "";

            return this._renderHtml.replace(new RegExp(pattern), val);
        }
        else
...
exports.Google = require("./elements/google");
