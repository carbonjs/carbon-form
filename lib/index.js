...
Form.prototype.addSubForm = function(name, form) {
    ...
    var recurseElements = function(elements) {
        _.forEach(elements, function(element) {
            if (element instanceof Form.Element)
            {
                var belongsTo = element.getBelongsTo();

                belongsTo = name + (belongsTo && belongsTo.length ? "." + belongsTo : "");
    ...
    recurseElements(form.getElements());
...
Form.prototype.getSubForm = function(name) {
    return _.get(this._subforms, name, null);
};
...
Form.prototype.isValid = function(values, callback) {
    ...
    _.forEach(this._subforms, function(subform, name) {
        functions.push(function(callback) {
            subform.isValid(values, function(err, values) {
                callback(err, values);
            });
        });
    });
